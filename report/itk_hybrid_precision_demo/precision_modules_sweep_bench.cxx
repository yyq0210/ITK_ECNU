...
main(int argc, char ** argv)
{
...

  {
    ...
    Bench(
      ...
      "AntiAliasBinary",
      [&]() {
        using F = itk::AntiAliasBinaryImageFilter<FImg, FImg>;
        auto f = F::New();
        f->SetInput(binF);
        f->SetMaximumRMSError(0.07);
      ...
      [&]() {
        using Cast = itk::CastImageFilter<FImg, DImg>;
        using F = itk::AntiAliasBinaryImageFilter<DImg, DImg>;
        auto c = Cast::New();
        c->SetInput(binF);
        auto f = F::New();
        f->SetInput(c->GetOutput());
...

  {
    ...
    Bench(
      ...
      "FastMarching",
      [&]() {
        using F = itk::FastMarchingImageFilter<FImg, FImg>;
        using Node = typename F::NodeType;
        using Nodes = typename F::NodeContainer;
        auto seeds = Nodes::New();
        Node node;
        node.SetValue(0.0f);
        node.SetIndex(seed);
        seeds->Initialize();
        ...
        auto f = F::New();
        f->SetInput(input);
        f->SetTrialPoints(seeds);
        f->SetOutputSize(sz);
        f->SetStoppingValue(50.0);
        f->Update();
        return f->GetOutput();
      },
      [&]() {
        using Cast = itk::CastImageFilter<FImg, DImg>;
        using F = itk::FastMarchingImageFilter<DImg, DImg>;
        using Node = typename F::NodeType;
        using Nodes = typename F::NodeContainer;
        auto c = Cast::New();
        c->SetInput(input);
