...
BIN=/home/pub/yyq/itk_hybrid_precision_demo/build/precision_metric_bench
OUTB=/tmp/metric_1024_bound.csv
OUTU=/tmp/metric_1024_unbound.csv
...
run_list() {
  ...
  for op in "${OPS[@]}"; do
    [[ -z "$op" ]] && continue
    echo "== $op ==" >&2
    if ! timeout 600 $wrap "$BIN" "$IMG" 3 "$op" >> "$out" 2>/tmp/metric_one.err; then
      echo "misc,$op,FAIL,FAIL,FAIL,timeout_or_crash," >> "$out"
      tail -6 /tmp/metric_one.err >&2 || true
    fi
  done
}

echo "===== METRIC BOUND $(date) ====="
run_list "taskset -c 0-95" "$OUTB"
echo "===== METRIC UNBOUND $(date) ====="
run_list "" "$OUTU"
echo "===== METRIC_DONE $(date) ====="
wc -l "$OUTB" "$OUTU"
