...
cd "$DEMO"
# keep existing fill_missing binary for the 1024 job; only build construct_bench
env -i HOME="$HOME" PATH="$PATH" \
  ...
  -DITK_DIR="$ITK"
cmake --build build --target precision_construct_bench -j8
echo BUILD_OK
./build/precision_construct_bench --list | wc -l
