#!/bin/bash
# 只跑 construct_bench 里比旧 fill_missing 多出来的算子；1024²；绑核+未绑核
set -u
...
OUTB=/tmp/construct_1024_bound.csv
OUTU=/tmp/construct_1024_unbound.csv

mapfile -t EXTRA < <(comm -13 <("$OLD" --list | sort) <("$NEW" --list | sort))
echo "extra_ops=${#EXTRA[@]}"

skip_serial() {
  case "$1" in
    *ByReconstruction*|*HMaxima*|*HMinima*|*HConvex*|*HConcave*|*Fillhole*|*GrindPeak*|*RegionalMaxima*|*RegionalMinima*|*BinaryThinning*|*BinaryPruning*|*FastMarching*|*ConnectedThreshold*|*ConfidenceConnected*|*IsolatedConnected*|*NeighborhoodConnected*)
      return 0 ;;
  esac
  return 1
}
...
run_list() {
  ...
  for op in "${EXTRA[@]}"; do
    ...
    echo "== extra $op ==" >&2
    if ! timeout 240 $wrap "$NEW" "$IMG" 3 "$op" >> "$out" 2>/tmp/construct_one.err; then
      echo "misc,$op,FAIL,FAIL,FAIL,timeout_or_crash," >> "$out"
      tail -2 /tmp/construct_one.err >&2 || true
    fi
  done
}

echo "===== CONSTRUCT BOUND $(date) ====="
run_list "taskset -c 0-95" "$OUTB"
echo "===== CONSTRUCT UNBOUND $(date) ====="
run_list "" "$OUTU"
echo "===== CONSTRUCT_DONE $(date) ====="
wc -l "$OUTB" "$OUTU"
